...
HARD_CASE_ENV_ACTIVATE="${HARD_CASE_ENV_ACTIVATE:-$QWEN_ENV_ACTIVATE}"
...
Environment overrides:
  ...
  HARD_CASE_ENV_ACTIVATE=$HARD_CASE_ENV_ACTIVATE
...
validate_paths() {
  ...
  [[ -f "$HARD_CASE_ENV_ACTIVATE" ]] || { echo "HARD_CASE_ENV_ACTIVATE does not exist: $HARD_CASE_ENV_ACTIVATE" >&2; exit 1; }
...
cd $(q "$PROJECT")
$(activation_line "$HARD_CASE_ENV_ACTIVATE")
export CUDA_VISIBLE_DEVICES=$(q "$AUDIO_GPU")
export TOKENIZERS_PARALLELISM=false
export HF_HUB_OFFLINE=1
export TRANSFORMERS_OFFLINE=1
