...
def _allowed(entity_type: str, target_entities: set[str]) -> bool:
    return not target_entities or str(entity_type).strip().upper() in target_entities


def _regex_detect(text: str, target_entities: set[str] | None = None) -> list[PIIEntity]:
    target_entities = target_entities or set()
    entities: list[PIIEntity] = []

    for match in re.finditer(r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b", text):
        if _allowed("EMAIL_ADDRESS", target_entities):
            entities.append(_entity("EMAIL_ADDRESS", text, match.start(), match.end(), 0.95))

    for match in re.finditer(r"(?<!\d)(?:\+?86[-\s]?)?1[3-9]\d{9}(?!\d)", text):
        if _allowed("CN_PHONE_NUMBER", target_entities):
    ...
    for match in re.finditer(us_phone, text):
        if _allowed("PHONE_NUMBER", target_entities):
            entities.append(_entity("PHONE_NUMBER", text, match.start(), match.end(), 0.82))

    for match in re.finditer(r"(?<!\d)0\d{2,3}[-\s]?\d{7,8}(?!\d)", text):
        if _allowed("PHONE_NUMBER", target_entities) and _has_context(text, match.start(), match.end(), ZH_CONTEXT["phone"]):
    ...
    for match in re.finditer(cn_id, text):
        if _allowed("CN_ID_CARD", target_entities) and _is_valid_cn_id_card(match.group(0)):
    ...
    for entity_type, pattern, score in contextual_patterns:
        if not _allowed(entity_type, target_entities):
            continue
    ...
    for match in re.finditer(r"[\u4e00-\u9fa5][A-Z][A-Z0-9]{5,6}", text):
        if _allowed("LICENSE_PLATE", target_entities):
    ...
    for match in re.finditer(address_pattern, text):
        if _allowed("LOCATION", target_entities):
...
def _run_presidio(text: str, settings: Settings, languages: list[str], target_entities: set[str] | None = None) -> tuple[list[PIIEntity], bool]:
    ...
    for language in languages:
        try:
            ...
            if target_entities:
                kwargs["entities"] = sorted(target_entities)
            rows = engine.analyze(
                **kwargs,
            )
        except Exception as exc:
...
def _gliner_labels(settings: Settings, target_entities: set[str] | None = None) -> list[str]:
    if settings.pii_gliner_labels.strip():
        labels = [label.strip() for label in settings.pii_gliner_labels.split(",") if label.strip()]
    else:
        labels = list(GLINER_LABEL_TO_ENTITY)
    target_entities = target_entities or set()
    ...
    return [
        label
        for label in labels
        if GLINER_LABEL_TO_ENTITY.get(label.strip().lower(), "").upper() in target_entities
    ]


def _run_gliner(text: str, settings: Settings, target_entities: set[str] | None = None) -> tuple[list[PIIEntity], bool]:
    if not settings.pii_enable_gliner:
        return [], False
    labels = _gliner_labels(settings, target_entities)
    if not labels:
        return [], False
    ...
    try:
        raw_entities = model.predict_entities(clipped, labels, threshold=settings.pii_gliner_threshold)
    except Exception as exc:
...
def detect(text: str, settings: Settings, language: str = "", target_entity_types: list[str] | None = None) -> LocalPIIDetection:
    ...
    if settings.pii_enable_regex_rules:
        ...
        entities.extend(_regex_detect(text, target_entities))

    if settings.pii_enable_presidio:
        providers.append("presidio-stanza")
        presidio_entities, presidio_degraded = _run_presidio(text, settings, languages, target_entities)
    ...
    if settings.pii_enable_gliner:
        ...
        gliner_entities, gliner_degraded = _run_gliner(text, settings, target_entities)
    ...
    if target_entities:
        filtered = [entity for entity in filtered if str(entity.entity_type).strip().upper() in target_entities]
