...
def moderate_endpoint(text: str, settings: Settings, target_labels: list[str] | None = None) -> QwenGuardResult | None:
    ...
    response = httpx.post(
        ...
        json={
            ...
            "target_labels": [str(item) for item in (target_labels or []) if str(item).strip()],
        },
...
def moderate_local(text: str, settings: Settings, target_labels: list[str] | None = None) -> QwenGuardResult:
    import torch

    model, tokenizer, device = load_model(settings)
    labels = [str(item).strip() for item in (target_labels or []) if str(item).strip()]
    label_hint = (
        "Focus only on these selected policy labels: " + ", ".join(labels) + ". "
        if labels
        else ""
    )
    prompt = (
...
def moderate(text: str, settings: Settings, target_labels: list[str] | None = None) -> QwenGuardResult:
    endpoint_result = moderate_endpoint(text, settings, target_labels=target_labels)
    if endpoint_result is not None:
        return endpoint_result
    return moderate_local(text, settings, target_labels=target_labels)
