...
from audio.models.schemas import AudioRunSummaryRecord, Decision, PrivacyResult, TranscriptUnit
from audio.operator_selection import resolve_audio_operator_selection
...
class AudioCompliancePipeline:
    ...
    def execute(self, input_paths: list[str]) -> ComplianceOutput:
        ...
        operator_selection = resolve_audio_operator_selection(self.settings)
        ...
        privacy_results = []
        redaction_spans = []
        if operator_selection.privacy_enabled:
            privacy_results, redaction_spans = f_privacy_detection.run(
                transcript_units,
                self.settings,
                target_entity_types=operator_selection.privacy_entity_types,
            )
        _write_jsonl(privacy_results, artifact_paths["privacy"])
        _write_jsonl(redaction_spans, artifact_paths["redaction_spans"])

        safety_inputs = _privacy_passthrough(transcript_units)
        safety_results = []
        if operator_selection.content_safety_enabled:
            safety_results = g_safety_moderation.run(
                safety_inputs,
                self.settings,
                target_labels=operator_selection.content_safety_target_labels,
            )
        ...
        summary_record = AudioRunSummaryRecord(
            ...
            metadata={
                "active_detectors": _active_detectors(operator_selection),
                "hard_case_units": len(hard_case_results),
                "reserved_extension_events": len(extension_events),
                "operator_selection": operator_selection.model_dump(),
                "pre_detection_pruning": {
                    "privacy": operator_selection.privacy_enabled,
                    "content_safety": operator_selection.content_safety_enabled,
                },
            },
        )
        ...
        *,
        ...
        return ComplianceOutput(
            ...
            metadata={
                ...
                "active_detectors": summary_record.metadata.get("active_detectors", ["privacy", "content_safety", "hard_case_adjudication"]),
                "operator_selection": summary_record.metadata.get("operator_selection", {}),
                "extension_interface": "AudioCompliancePipeline._run_detection_extensions",
            },
        )


def _privacy_passthrough(units: list[TranscriptUnit]) -> list[PrivacyResult]:
    return [
        PrivacyResult(
            ...
            provider_name="privacy_pruned",
            provider_version="operator_selection",
            is_degraded=False,
        )
        for unit in units
    ]
...
def _active_detectors(operator_selection) -> list[str]:  # type: ignore[no-untyped-def]
    detectors: list[str] = []
    if operator_selection.privacy_enabled:
        detectors.append("privacy")
    if operator_selection.content_safety_enabled:
        detectors.append("content_safety")
    detectors.append("hard_case_adjudication")
    return detectors
