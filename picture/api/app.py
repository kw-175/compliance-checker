...
async def readiness() -> dict[str, Any]:
    """Runtime readiness for the local picture compliance model stack."""
    from picture.infra.model_readiness import check_picture_model_readiness

    readiness_report = check_picture_model_readiness()
    return {
        "status": "ready" if readiness_report["ready_for_full_runtime"] else "not_ready",
        "service": "picture-compliance-checker",
        "readiness": readiness_report,
    }


@app.get("/v1/picture/admin/runtime")
async def picture_runtime() -> dict[str, Any]:
    """Return cached provider and runtime configuration diagnostics."""
    from picture.application.use_cases import get_runtime_components
    from picture.infra.config import get_settings

    return {
        "service": "picture-compliance-checker",
        "runtime": get_runtime_components(get_settings()),
    }


@app.post("/v1/picture/admin/warmup")
async def picture_warmup() -> dict[str, Any]:
    """Warm local picture providers so first real jobs do not pay model load cost."""
    from picture.application.use_cases import warmup_runtime
    from picture.infra.config import get_settings

    return {
        "service": "picture-compliance-checker",
        "warmup": warmup_runtime(get_settings()),
    }
