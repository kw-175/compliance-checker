...
PICTURE_OCR_PROVIDER=mock       # mock | paddleocr | mineru | surya | openai_gpt52
PICTURE_PII_PROVIDER=mock       # mock | presidio | openai_gpt52
PICTURE_SAFETY_PROVIDER=mock    # mock | shieldgemma2 | openai_gpt52
PICTURE_VISION_PROVIDER=mock    # mock | yolo26 | grounding_dino | openai_gpt52
PICTURE_SEGMENTATION_PROVIDER=mock  # mock | sam2
PICTURE_STORAGE_BACKEND=local   # local | s3
PICTURE_SERVER_PORT=19012
PICTURE_OPENAI_API_KEY=...
PICTURE_OPENAI_MODEL=gpt-5.2
PICTURE_OPENAI_BASE_URL=https://api.openai.com/v1
```
...
`picture/` 现在支持一条基于 `GPT-5.2` 的外部 API 图片合规检测路线：

- OCR、PII、安全审核、视觉检测统一走 `openai_gpt52`
- 单张图片只调用一次外部 API
- 本地继续复用现有 `policy`、`report`、`redaction` 与 HTTP API

建议至少设置：

```powershell
$env:OPENAI_API_KEY = "your-openai-key"
$env:PICTURE_OPENAI_MODEL = "gpt-5.2"
```

如果你使用兼容网关，也可以覆盖：

```powershell
$env:PICTURE_OPENAI_BASE_URL = "https://api.openai.com/v1"
```
...
### 创建任务
```bash
curl -X POST http://localhost:19012/v1/picture/jobs \
...
### 查询状态
```bash
curl http://localhost:19012/v1/picture/jobs/{job_id}
```

### 获取结果
```bash
curl http://localhost:19012/v1/picture/jobs/{job_id}/result
```

### 获取 Findings
```bash
curl http://localhost:19012/v1/picture/jobs/{job_id}/findings
```
...
# 或者使用 uvicorn
uvicorn picture.api.app:app --host 0.0.0.0 --port 19012 --reload
```

服务启动后访问 `http://localhost:19012/docs` 查看 OpenAPI 文档。

## 一键启动与一键测试（PowerShell）

Windows PowerShell 下可直接使用这两个脚本：

- [start_openai_api.ps1](/mnt/d/CodeVS/CodePython/compliance-checker/picture/start_openai_api.ps1)
- [test_openai_api.ps1](/mnt/d/CodeVS/CodePython/compliance-checker/picture/test_openai_api.ps1)

### 一键启动

在 `compliance-checker` 根目录执行：

```powershell
$env:OPENAI_API_KEY = "your-openai-key"
.\picture\start_openai_api.ps1
```

默认行为：

- 自动使用 `.venv\Scripts\python.exe`
- 自动把 OCR / PII / Safety / Vision provider 切到 `openai_gpt52`
- 默认监听 `http://127.0.0.1:19012`

常见可选参数：

```powershell
.\picture\start_openai_api.ps1 -BindHost 0.0.0.0 -Port 19012
.\picture\start_openai_api.ps1 -Reload
```
...
```powershell
.\picture\test_openai_api.ps1
```

脚本会自动：

- 检查 `/api/v1/health`
- 自动生成一张带手机号、邮箱、姓名、地址的文档探针图
- 预期结果：`DONE / pass_redacted`
- 提交 `picture\tests\fixtures\sample_unsafe_explicit.png`
- 预期结果：`DROPPED / drop`

如果服务地址不是默认值，可传：

```powershell
.\picture\test_openai_api.ps1 -BaseUrl "http://127.0.0.1:19012"
```
