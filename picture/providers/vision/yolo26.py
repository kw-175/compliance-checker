    ...
    def _get_model(self) -> Any:
        ...
        if self._model is None:
            try:
                ...
                if not str(self._model_path).strip():
                    raise ProviderNotAvailableError("YOLO local model file is required")
                model_path = Path(self._model_path)
                if not model_path.is_file():
                    raise ProviderNotAvailableError(
                        f"YOLO local model file is required, got {self._model_path!r}"
                    )
